defmodule Bloom.Eth do
  ...
  def describe(<<"0x", txid::binary-size(64)>>) do
  ...
  defp account_balancemulti(addresses),
    do: etherscan_request("account", "balancemulti", address: Enum.join(addresses, ","), tag: "latest")

  defp etherscan_request(module, action, args) do
    with kws = [module: module, action: action] ++ args,
         query = kws |> Enum.map(fn {k, v} -> "#{k}=#{v}" end) |> Enum.join("&"),
         url = @endpoint <> "?" <> query,
